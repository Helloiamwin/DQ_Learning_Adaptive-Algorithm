...
save_check_path = "trained_models/save_test_17_12_{}.pickle"
save_check_point = []
...
def train(opt):
    ...
    #checkpoint_path = os.path.join(opt.saved_folder, "chrome_dino_{}.pth")
    checkpoint_path = os.path.join(opt.saved_folder, "chrome_dino_99999.pth")
    #memory_path = os.path.join(opt.saved_folder, "replay_memory_99999.pkl")
    ...
    while iter < opt.num_iters:
        ...
        if (iter + 1) % 2 == 0:
           save_check_point.append([iter, loss.item(), epsilon, reward])
        if (iter + 1) % 10000 == 0:
            with open(save_check_path.format(iter), "wb") as f:
                pickle.dump(save_check_point, f,protocol=pickle.HIGHEST_PROTOCOL)

        if (iter + 1) % 10000 == 0:
            checkpoint = {"iter": iter,
                          "model_state_dict": model.state_dict(),
                          "optimizer": optimizer.state_dict()}
            torch.save(checkpoint, checkpoint_path.format(iter))
            #torch.save(checkpoint, checkpoint_path)
            # with open(memory_path.format(iter), "wb") as f:
